...
【输出 JSON Schema】
{
  ...
  "visual_consistency": "一段英文角色锚点，描述四帧里必须保持一致的主角外貌、年龄、发型、服装主色、体型、气质和关键道具；如果没有固定主角，描述必须保持一致的核心物体或地点。",
  "storyboard": [
...
- 必须先写 `visual_consistency` 顶层字段，用英文固定主角或核心物体：年龄段、发型、脸型、服装主色、身形、气质、关键道具。四个 frame prompt 里也要用同一个称呼重复这个主角，不要每帧重新发明人物。
...
def validate_payload(payload: dict, lesson_id: int) -> list[str]:
    ...
    # storyboard 字段
    visual_consistency = payload.get("visual_consistency")
    if visual_consistency is not None and not isinstance(visual_consistency, str):
        errors.append("visual_consistency 必须是字符串")
